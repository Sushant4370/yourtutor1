...
const fulfillBooking = async (session: Stripe.Checkout.Session) => {
    ...
    const availabilitySlotId = session.metadata?.availabilitySlotId;
    ...
    try {
        ...
        // 0. REMOVE AVAILABILITY SLOT (ID-based method)
            ...
            try {
                const updateResult = await TutorProfileModel.updateOne(
                    { tutorId: booking.tutorId },
                    {
                        $pull: {
                            availability: {
                                _id: new mongoose.Types.ObjectId(availabilitySlotId)
                            }
                        }
                    }
                );
                ...
                if (updateResult.modifiedCount > 0) {
                    console.log(`- [Webhook] Availability slot ${availabilitySlotId} removed for tutor ${booking.tutorId}.`);
                    revalidatePath(`/tutors/${booking.tutorId.toString()}`);
                } else {
                    console.warn(`- [Webhook] Could not find matching availability slot with ID ${availabilitySlotId} to remove for tutor ${booking.tutorId}. It might have been removed already.`);
                }
            } catch (availError: any) {
                console.error(`❌ [Webhook] CRITICAL: Failed to remove availability for booking ${bookingId}. Error: ${availError.message}.`);
            }
        } else {
            console.warn(`- [Webhook] fulfillBooking: Missing availabilitySlotId in session metadata for session ${session.id}. Cannot remove availability slot.`);
        }
